...
int _find_s(int _step, int _sum1, int _sum2) {
	...
	if (_DP_s[_step][_sum1 + 5000] != -5001) {
		_sum2 += _DP_s[_step][_sum1 + 5000];
		_result = _sum2;
		_update();
		return _sum2;
	}
	...
	for (s[_step] = 0; s[_step] <= 1; s[_step]++) {
		...
		if (_tmp0 > _DP_s[_step][__sum1 + 5000])
			_DP_s[_step][__sum1 + 5000] = _tmp0;
	}
	return __sum2 + _DP_s[_step][__sum1 + 5000];
}
